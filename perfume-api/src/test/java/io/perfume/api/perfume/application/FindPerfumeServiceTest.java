...
class FindPerfumeServiceTest {
  ...
  @InjectMocks private FindPerfumeService findPerfumeService;
  @Mock private PerfumeQueryRepository perfumeQueryRepository;
  @Mock private FindCategoryUseCase findCategoryUseCase;
  @Mock private FindBrandUseCase findBrandUseCase;
  ...
  void findPerfumeById() {
    ...
    given(perfumeQueryRepository.findPerfumeById(anyLong()))
        .willReturn(Optional.ofNullable(perfume));
    CategoryResult category =
        new CategoryResult(
            1L, "시트러스", "상큼하고 톡 쏘는 향으로 가볍고 산뜻한 느낌을 줍니다.", "#상큼한 #질리지않는", "thumbnail.com");
    given(findCategoryUseCase.findCategoryById(anyLong())).willReturn(category);
    ...
    given(findFileUseCase.findFileById(anyLong()))
        .willReturn(Optional.ofNullable(File.builder().url("thumbnail.com").build()));
    // when
