...
@Import({
  PerfumeFavoriteQueryPersistenceAdapter.class,
  PerfumeFavoriteMapper.class,
...
class PerfumeFavoriteQueryPersistenceAdapterTest {
  ...
  @Autowired private PerfumeFavoriteQueryPersistenceAdapter queryRepository;

  @Autowired private PerfumeFavoriteMapper perfumeFavoriteMapper;

  @Autowired private PerfumeFavoriteJpaRepository jpaRepository;
  ...
  void testFindByUserAndPerfume() {
    ...
    // when
    PerfumeFavorite followed =
        queryRepository
            .findByUserIdAndPerfumeId(entity.getId().getUserId(), entity.getId().getPerfumeId())
            .orElseThrow();
  ...
  void testFindFollowedPerfumesByUser() {
    ...
    for (int i = start; i < end; i++) {
      var follow = PerfumeFavorite.create(userId, perfumeId++, now);
    ...
    assertThat(followedPerfumes)
        .allMatch(perfumeFollow -> perfumeFollow.getUserId().equals(userId));
