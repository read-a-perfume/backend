...
//  }
  ...
  void reviewCount() {
    // given
    PerfumeJpaEntity perfumeJpaEntity = PerfumeJpaEntity.builder()
        .name("perfume")
        .story("story")
    ...
    for (int i = 0; i < 10; i++) {
      Review review = Review.create("좋아요", "그냥 제 취향입니다.", Strength.LIGHT, Duration.LONG, DayType.DAILY, 1L, 1L, Season.SPRING, now);
      entityManager.persist(reviewMapper.toEntity(review));
    }
    for (int i = 0; i < 5; i++) {
      Review review = Review.create("좋아요", "그냥 제 취향입니다.", Strength.MODERATE, Duration.MEDIUM, DayType.TRAVEL, 1L, 2L, Season.SPRING, now);
      entityManager.persist(reviewMapper.toEntity(review));
    }
    for (int i = 0; i < 3; i++) {
      Review review = Review.create("좋아요", "그냥 제 취향입니다.", Strength.HEAVY, Duration.SHORT, DayType.SPECIAL, 1L, 1L, Season.SUMMER, now);
      entityManager.persist(reviewMapper.toEntity(review));
    }
    entityManager.flush();
    entityManager.clear();
