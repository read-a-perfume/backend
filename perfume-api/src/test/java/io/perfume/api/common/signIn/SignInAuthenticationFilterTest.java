...
class SignInAuthenticationFilterTest {
    ...
    void setUp(WebApplicationContext webApplicationContext, RestDocumentationContextProvider restDocumentation) {
        filter = new SignInAuthenticationFilter(authenticationManagerBuilder.getOrBuild(), makeNewTokenUseCase);
    ...
    void successLoginGenerateToken() throws Exception {
        ...
        when(makeNewTokenUseCase.createAccessToken(anyLong(), any(LocalDateTime.class))).thenReturn("Bearer SampleAccessToken");
...
//        userRepository.save(createUser(email, password));
    ...
    private User createUser(String email, String password) {
        ...
        return User.builder().id(1L).username("test").email(email).name("test").role(Role.USER).password(encodedPassword).role(Role.USER).build();
    }
}
