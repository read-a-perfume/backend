...
public record ReviewDetailResult(
  ...
  public static ReviewDetailResult from(
      ReviewResult review, UserResult user, List<String> tags, long likeCount, long commentCount) {
    final var reviewDetailUserResult = ReviewDetailUserResult.from(user);

    return new ReviewDetailResult(
        review.id(), review.shortReview(), reviewDetailUserResult, tags, likeCount, commentCount);
  }
}
