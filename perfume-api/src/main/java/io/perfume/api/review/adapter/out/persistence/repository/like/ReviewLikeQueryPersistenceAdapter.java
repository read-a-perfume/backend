...
public class ReviewLikeQueryPersistenceAdapter implements ReviewLikeQueryRepository {
  ...
  public List<ReviewLikeCount> countByReviewIds(List<Long> reviewIds) {
    return jpaQueryFactory
        .select(
            QReviewLikeEntity.reviewLikeEntity.reviewId, QReviewLikeEntity.reviewLikeEntity.count())
        ...
        .filter(
            tuple ->
                tuple.get(QReviewLikeEntity.reviewLikeEntity.reviewId) != null
                    && tuple.get(QReviewLikeEntity.reviewLikeEntity.count()) != null)
        .map(
            tuple ->
                new ReviewLikeCount(
                    tuple.get(QReviewLikeEntity.reviewLikeEntity.reviewId),
                    tuple.get(QReviewLikeEntity.reviewLikeEntity.count())))
        .toList();
  }
}
