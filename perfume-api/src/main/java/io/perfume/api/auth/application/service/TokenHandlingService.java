...
public class TokenHandlingService implements MakeNewTokenUseCase {
     ...
     */
    @Override
    public ReissuedTokenResult reissueAccessToken(String accessToken, String refreshToken, LocalDateTime now) {
        boolean isValidTokens = authenticationTokenService.isValid(refreshToken, now) && authenticationTokenService.isValid(accessToken, now);
        if (isValidTokens) {
            throw new IllegalArgumentException("Invalid tokens");
        }

        RefreshToken refreshTokenDomain = authenticationTokenService.getRefreshTokenFromClient(refreshToken);
    ...
    @NotNull
    private ReissuedTokenResult getReissuedTokenResult(String accessToken, RefreshToken newRefreshToken,
                                                       LocalDateTime now) {
    ...
    public String createRefreshToken(Long userId, LocalDateTime now) {
        ...
        return authenticationTokenService.createRefreshToken(refreshToken.getTokenId(), refreshToken.getUserId(), now);
    }
